"""Run PRnet-style Sci-Plex 3 pseudobulk main30 benchmark."""
...
def run_one(data: dict[str, object], seed: int, split_name: str) -> tuple[dict[str, object], pd.DataFrame, pd.DataFrame, str]:
    ...
    pred_path = PRED / f"prnet_main30_seed{seed:03d}_{split_name}.npz"
...
def main() -> None:
    ...
    for seed in SEEDS:
        for split_name in SPLITS:
            ...
            pd.DataFrame(metrics).to_csv(OUT / "prnet_main30_metrics_long.csv", index=False)
            (pd.concat(manifests, ignore_index=True) if manifests else pd.DataFrame()).to_csv(
                OUT / "prnet_main30_predictions_manifest.csv", index=False
            )
            pd.concat(leakages, ignore_index=True).to_csv(OUT / "prnet_main30_leakage_audit.csv", index=False)
    metrics_df = pd.DataFrame(metrics)
    contrast_table(metrics_df).to_csv(OUT / "prnet_main30_random_to_strict_contrasts.csv", index=False)
