"""Summarize PRnet main10, integrate model panel outputs, and plot figures."""
...
def paired_differences(prnet: pd.DataFrame, transigen: pd.DataFrame, baseline: pd.DataFrame) -> pd.DataFrame:
    ...
    out.to_csv(SCI / "prnet_main10_paired_differences.csv", index=False)
    return out


def build_rank_seed_table(prnet: pd.DataFrame, transigen: pd.DataFrame, baseline: pd.DataFrame) -> pd.DataFrame:
    base = baseline[baseline["seed"].between(1, 10) & baseline["baseline"].isin(BASELINE_TO_MODEL)].copy()
    ...
    all_rows.to_csv(SCI / "prnet_main10_rank_by_seed.csv", index=False)
    return all_rows
        ...
        rows.append(
            {
                ...
                "status": "prnet_main10_seed1_10_completed",
            }
        )
    out = pd.DataFrame(rows)
    return out
...
def update_panel_outputs(summary: pd.DataFrame, rank_rows: pd.DataFrame) -> None:
    ...
    prnet_rows["notes"] = "PRnet-style Sci-Plex 3 pseudobulk adaptation main10; not a full PRnet reproduction."
    ...
    prnet_con = pd.read_csv(SCI / "prnet_main10_random_to_strict_contrasts.csv")
...
def plot_figures(summary: pd.DataFrame, paired: pd.DataFrame, rank_seed: pd.DataFrame, rank_rows: pd.DataFrame) -> None:
    ...
    prnet = pd.read_csv(SCI / "prnet_main10_metrics_long.csv")
    plot_rows = []
    for model in ["global_train_mean", "ridge_drug_fp", "ridge_cell_dose_drug_fp"]:
        sub = base[base["seed"].between(1, 10) & base["baseline"].eq(model) & base["split_label"].isin(["random", "scaffold_heldout", "cell_scaffold_heldout"])]
    ...
    for model_name, df in [(TRANSIGEN, trans), (PRNET, prnet)]:
        sub = df[df["seed"].between(1, 10) & df["status"].eq("completed") & df["split_type"].isin(["random", "scaffold_heldout", "cell_scaffold_heldout"])]
    ...
    panel.to_csv(SRC / "figure6_sciplex3_prnet_transigen_model_panel.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(FIG / "figure6_sciplex3_prnet_transigen_model_panel.png", dpi=300, bbox_inches="tight")
    fig.savefig(FIG / "figure6_sciplex3_prnet_transigen_model_panel.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    rank_heat.to_csv(SRC / "figure7_rank_transfer_with_prnet.csv", index=False)
    ...
    rank_rows.to_csv(SRC / "figure7_rank_transfer_with_prnet_summary.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(FIG / "figure7_rank_transfer_with_prnet.png", dpi=300, bbox_inches="tight")
    fig.savefig(FIG / "figure7_rank_transfer_with_prnet.pdf", bbox_inches="tight")
    plt.close(fig)

    prnet.to_csv(SRC / "supp_prnet_seed_distribution.csv", index=False)
    ...
    ax.set_ylabel("All-gene row-wise Pearson")
    ax.set_title("PRnet adapted seed-level distribution")
    fig.tight_layout()
    fig.savefig(FIG / "supp_prnet_seed_distribution.png", dpi=300, bbox_inches="tight")
    fig.savefig(FIG / "supp_prnet_seed_distribution.pdf", bbox_inches="tight")
    plt.close(fig)

    pvt = paired[paired["comparison_model"].eq(TRANSIGEN)].copy()
    pvt.to_csv(SRC / "supp_prnet_vs_transigen_paired.csv", index=False)
    ...
    ax.set_xlabel("PRnet minus TranSiGen Pearson")
    ax.set_title("Paired PRnet vs TranSiGen difference")
    fig.tight_layout()
    fig.savefig(FIG / "supp_prnet_vs_transigen_paired.png", dpi=300, bbox_inches="tight")
    fig.savefig(FIG / "supp_prnet_vs_transigen_paired.pdf", bbox_inches="tight")
    plt.close(fig)


def write_report(summary: pd.DataFrame, paired: pd.DataFrame, rank_rows: pd.DataFrame) -> None:
    vals = summary.set_index("split_type")
    completed = int(summary["seeds"].min())
    report = f"""# PRnet Main10 Run Report
...
- Successful seed/split combinations: {completed}/10 for every required split.
...
|---|---:|---:|---:|---:|
| Random | {int(vals.loc['random', 'seeds'])} | {vals.loc['random', 'mean_all_gene_pearson']:.4f} | {vals.loc['random', 'sd_all_gene_pearson']:.4f} | {vals.loc['random', 'mean_all_gene_rmse']:.4f} |
| Scaffold held-out | {int(vals.loc['scaffold_heldout', 'seeds'])} | {vals.loc['scaffold_heldout', 'mean_all_gene_pearson']:.4f} | {vals.loc['scaffold_heldout', 'sd_all_gene_pearson']:.4f} | {vals.loc['scaffold_heldout', 'mean_all_gene_rmse']:.4f} |
| Joint held-out | {int(vals.loc['cell_scaffold_heldout', 'seeds'])} | {vals.loc['cell_scaffold_heldout', 'mean_all_gene_pearson']:.4f} | {vals.loc['cell_scaffold_heldout', 'sd_all_gene_pearson']:.4f} | {vals.loc['cell_scaffold_heldout', 'mean_all_gene_rmse']:.4f} |

Random minus scaffold Pearson difference: {(vals.loc['random', 'mean_all_gene_pearson'] - vals.loc['scaffold_heldout', 'mean_all_gene_pearson']):.4f}.

Random minus joint Pearson difference: {(vals.loc['random', 'mean_all_gene_pearson'] - vals.loc['cell_scaffold_heldout', 'mean_all_gene_pearson']):.4f}.
...
"""
    (ROOT / "docs/prnet_main10_run_report.md").write_text(report, encoding="utf-8")
...
def main() -> None:
    ...
    summary.to_csv(SCI / "prnet_main10_summary_stats.csv", index=False)
    ...
    write_report(summary, paired, rank_rows)
